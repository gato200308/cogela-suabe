...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $contrasena = trim($_POST['contrasena'] ?? ''); // Corregirel uso de la función trim  // Validar que no estén vacíos
    if (empty($identificacion) || empty($correo) || empty($contrasena)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            if (password_verify($contrasena, $stored_hash)) {
                ...
                $_SESSION['user_id'] = $identificacion; // Asegurar que user_id esté configurado
